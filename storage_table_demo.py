...
resourcegroup_name = 'jpa'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))

storageaccount_name = 'jpa'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
...
car.price= 18577.00
table_service.insert_entity('itemstable', car)
print('Created entry for Chevy Malibu...\n')
time.sleep(1)
...
menuItem = Entity()
menuItem.PartitionKey = 'cafeProducts'
menuItem.RowKey = '004'
menuItem.brand = 'Mocha'
menuItem.flavor = 'Peppermint'
menuItem.size = 'Large'
menuItem.price = 4.50
table_service.insert_entity('itemstable', menuItem)
...
menuItem.PartitionKey = 'cafeProducts'
menuItem.RowKey = '005'
menuItem.brand = 'Cappuccino'
menuItem.flavor = 'Caramel'
menuItem.size = 'Large'
menuItem.price = 3.50
table_service.insert_entity('itemstable', menuItem)
print('Created entry for a Caramel Cappucino...\n')
time.sleep(1)

menuItem = Entity()
menuItem.PartitionKey = 'cafeProducts'
menuItem.RowKey = '006'
menuItem.brand = 'Latte'
menuItem.flavor = 'Vanilla'
menuItem.size = 'Large'
menuItem.price = 4.00
table_service.insert_entity('itemstable', menuItem)
print('Created entry for a Vanilla Latte...\n')
time.sleep(1)

###
# Use the Azure Storage Storage SDK for Python to query for entities in our Table
###
print('With some data in our Azure Storage Table, we can query the data.\nLet\'s see what the automobile  inventory looks like.')
raw_input('Press Enter to continue...')
...
items = table_service.query_entities(
    'itemstable', 
    filter="PartitionKey eq 'carInventory'", 
    select='make,model,year,color,price')
for item in items:
    print('Vehicle: ' + str(item.year) + ' ' + item.make + ' ' + item.model + '\n')
    print('Color: ' + item.color + '\n')
    print('Price: ' + str(item.price) + '\n')

items = table_service.query_entities(
    'itemstable', 
    filter="PartitionKey eq 'cafeProducts'", 
    select='brand,flavor,size,price')
for item in items:
    print('Item: ' + item.flavor + ' ' + item.brand + '\n')
